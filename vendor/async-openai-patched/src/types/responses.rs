...
pub struct CreateResponse {
    ...
    /// - `reasoning.encrypted_content` Include an encrypted version of
    ...
    ///   API statelessly (for example, when the `store` parameter is set to
    ...
    /// Supported values:
    /// - `auto`
    ///   - If the Project is Scale tier enabled, the system will utilize Scale
    ///     tier credits until they are exhausted.
    ///   - If the Project is not Scale tier enabled, the request will be
    ///     processed using the default service tier with a lower uptime SLA
    ///     and no latency guarantee.
    /// - `default` The request will be processed using the default service
    ///   tier with a lower uptime SLA and no latency guarantee.
    /// - `flex` The request will be processed with the Flex Processing service
    ///   tier. Learn more.
    ///
    ...
    /// 10% probability mass are considered. We generally recommend
...
pub struct PromptConfig {
    ...
    /// The substitution values can either be strings, or other Response
    /// input types like images or files. For now only supporting Strings.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub enum ToolDefinition {
    ...
    LocalShell,
    /// `type: custom` tools (e.g. Cursor) — opaque JSON, forwarded to the
    /// provider.
    #[serde(rename = "custom")]
    Custom(serde_json::Value),
}
...
pub enum ToolChoiceMode {
    ...
    None,
    /// The model can pick between generating a message or calling one or more
    /// tools.
    Auto,
...
pub enum OutputContent {
    ...
    /// generating a response. Be sure to include these items in your input
    /// to the Responses API for subsequent turns of a conversation if you
    /// are manually managing context.
...
/// A reasoning item representing the model's chain of thought, including
/// summary paragraphs.
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq)]
...
pub struct FileSearchResult {
    ...
    /// Set of 16 key-value pairs that can be attached to an object. This can
    /// be useful for storing additional information about the object in a
    /// structured format, and querying for objects API or the dashboard.
    /// Keys are strings with a maximum length of 64 characters
    /// . Values are strings with a maximum length of 512 characters, booleans,
    /// or numbers.
...
pub struct Response {
    ...
    /// Text format configuration echoed back (plain, json_object,
    /// json_schema).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub text: Option<TextConfig>,
