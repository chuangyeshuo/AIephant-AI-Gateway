...
pub fn apply_alephant_cache_hit_headers(
    dst: &mut HeaderMap,
    bucket_idx: usize,
    latency_ms: u64,
) {
    ...
    if let Ok(v) = HeaderValue::try_from(bucket_idx.to_string()) {
        let _ =
            dst.insert(HeaderName::from_static("alephant-cache-bucket-idx"), v);
    }
    if let Ok(v) = HeaderValue::try_from(latency_ms.to_string()) {
        let _ =
            dst.insert(HeaderName::from_static("alephant-cache-latency"), v);
    }
}
