...
#[must_use]
pub fn default_named_provider_capabilities(
    provider_name: &str,
) -> ProviderCapabilities {
    default_provider_capabilities(&InferenceProvider::Named(
        provider_name.into(),
    ))
}
...
mod tests {
    fn assert_openai_compatible_passthrough_profile(provider_name: &str) {
        let capabilities =
            super::default_named_provider_capabilities(provider_name);
    ...
    fn default_named_provider_capabilities_unknown_falls_back_to_generic_profile()
     {
        let capabilities =
            super::default_named_provider_capabilities("custom-openai");
