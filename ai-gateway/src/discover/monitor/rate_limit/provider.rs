...
impl<K> ProviderMonitorInner<K> {
    ...
    fn is_rate_limit_failover_enabled(&self) -> bool {
        crate::fallback::evaluator::should_remove(
            &self.app_state.config().fallback_policy,
            crate::fallback::evaluator::FailoverSource::RateLimit,
        )
    }
}
    ...
    async fn monitor(
        ...
        loop {
            tokio::select! {
                ...
                Some(event) = rx.recv() => {
                    if !self.is_rate_limit_failover_enabled() {
                        debug!(
                            provider = ?event.api_endpoint.provider(),
                            api_endpoint = ?event.api_endpoint,
                            router_id = ?self.router_id,
                            "rate-limit failover disabled by policy; ignoring event"
                        );
                        continue;
                    }
                    ...
                    if rate_limited_providers.contains(&key) {
                        ...
                        crate::fallback::observability::log_decision(
                            ...
                            Some(crate::fallback::evaluator::FailoverSource::RateLimit),
                            &event.api_endpoint.provider(),
                        );
                        ...
                        let duration = fallback_bridge::resolved_rate_limit_restore_after(
