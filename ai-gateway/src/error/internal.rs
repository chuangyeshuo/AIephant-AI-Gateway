...
pub enum InternalError {
    ...
    /// Provider {0} is not in the workspace's allowed provider set
    ProviderNotAllowedForWorkspace(InferenceProvider),
    /// Content filter service unavailable: {0}
    ContentFilterUnavailable(String),
}
...
pub enum InternalErrorMetric {
    ...
    /// Database error
    DatabaseError,
    /// Provider not allowed for workspace
    ProviderNotAllowedForWorkspace,
    /// Content filter unavailable
