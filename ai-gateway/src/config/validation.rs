...
impl Config {
    ...
    fn can_map_model(
        ...
        if target_provider.skips_model_mapping_catalog_for_static_validation() {
            return Ok(());
        }
...
mod tests {
    ...
    #[test]
    fn can_map_model_skips_catalog_when_target_is_openrouter() {
        let config = Config::default();
        let router_config = RouterConfig {
            model_mappings: None,
            ..Default::default()
        ...
        let result = config.can_map_model(
            ...
            &target_models,
            &RouterId::Named(CompactString::new("r")),
            &router_config,
        );
