...
const MODEL_MAPPING_YAML: &str = include_str!("../../config/embedded/model-mapping.yaml");

#[derive(Debug, Clone, Deserialize, Serialize, AsRef, PartialEq, Eq)]
pub struct ModelMappingConfig(pub(crate) NEMap<ModelName<'static>, NESet<ModelId>>);

impl Default for ModelMappingConfig {
    #[allow(clippy::too_many_lines)]
    fn default() -> Self {
        serde_yml::from_str(MODEL_MAPPING_YAML).expect("Always valid if tests pass")
    }
}
