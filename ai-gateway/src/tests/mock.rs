...
impl Mock {
    ...
    pub async fn new(config: &mut Config, args: MockArgs) -> Self {
        ...
        let s3_mock = start_mock_for_test(
            &get_stubs_path("s3"),
            None,
            args.stubs.as_ref(),
            args.verify,
        ...
        .await;
        config.s3.endpoint = Url::parse(&s3_mock.uri()).unwrap();

        let alephant_mock = start_mock_for_test(
            &get_stubs_path("alephant"),
            None,
            args.stubs.as_ref(),
            args.verify,
            args.alephant_port,
    ...
    pub async fn verify(&self) {
        ...
        self.s3_mock.http_server.verify().await;
        self.alephant_mock.http_server.verify().await;
    ...
    pub async fn reset(&self) {
        ...
        self.s3_mock.http_server.reset().await;
        self.alephant_mock.http_server.reset().await;
...
async fn handle_presigned_url_mock(
        ...
        stubs.and_then(|s| s.get("success:alephant:sign_s3_url"))
    {
        ...
        // stubs here.
        let s3_base_url = s3_mock.http_server.uri();
